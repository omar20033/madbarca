document.addEventListener('DOMContentLoaded', function () {
  ...
  function verdictFor(pct) {
    if (pct === 100) return 'Puntuación perfecta — un auténtico experto.';
    if (pct >= 75) return 'Buen conocimiento de la rivalidad.';
    if (pct >= 50) return 'Correcto, pero hay margen de mejora.';
    if (pct >= 25) return 'Toca repasar la historia.';
    return 'Vuelve a las FAQs, anda.';
  }
  ...
  function renderQuestion() {
    ...
    nextBtn.textContent = (current === total - 1) ? 'Ver resultado →' : 'Siguiente pregunta →';
  ...
  function handleAnswer(i, btnEl) {
    ...
    explainEl.innerHTML = '<strong>' + (i === item.correct ? 'Correcto. ' : 'No exactamente. ') + '</strong>' + item.explain;
