document.addEventListener('DOMContentLoaded', function () {
  ...
  function verdictFor(pct) {
    if (pct === 100) return 'Perfect score — genuine expert.';
    if (pct >= 75) return 'Strong knowledge of the rivalry.';
    if (pct >= 50) return 'Solid, but room to grow.';
    if (pct >= 25) return 'Time to read up on the history.';
    return 'Back to the FAQ with you.';
  }
  ...
  function renderQuestion() {
    ...
    nextBtn.textContent = (current === total - 1) ? 'See Result →' : 'Next Question →';
